...
const DEFAULT_FAVICON = '/branding/sigil-favicon.svg';
const DEFAULT_LOGIN_LOGO_LIGHT = '/branding/sigil-logo.svg';
const DEFAULT_LOGIN_LOGO_DARK = '/branding/sigil-logo.svg';
...
describe('withBasePath — asset-URL fallbacks under a subpath', () => {
  ...
  it('prefixes the built-in favicon and login-logo defaults', async () => {
    ...
    expect(withBasePath(DEFAULT_FAVICON)).toBe('/webmail/branding/sigil-favicon.svg');
    expect(withBasePath(DEFAULT_LOGIN_LOGO_LIGHT)).toBe('/webmail/branding/sigil-logo.svg');
    expect(withBasePath(DEFAULT_LOGIN_LOGO_DARK)).toBe('/webmail/branding/sigil-logo.svg');
  });

  it('is idempotent for an already-prefixed value (no double prefix)', async () => {
    const { withBasePath } = await loadNav('/webmail');
    expect(withBasePath('/webmail/branding/sigil-favicon.svg')).toBe('/webmail/branding/sigil-favicon.svg');
